...
class CodeGraph:
    ...
    def neighborhood(self, module_name: str, depth: int = 1) -> "CodeGraph":
        """Returns a subgraph around a specific module."""
        subgraph = CodeGraph()
        
        if module_name not in self.nodes:
            # Let's see if there's a symbol by this name
            # For simplicity, if it's not a module name, we try to find the module that has the symbol.
            # We'll implement this search in the CLI. Here we assume module_name is valid.
            return subgraph
        ...
        # Add edges between all visited nodes
...
def build_graph(parsed_files: List[ParsedFile], resolved_map: Dict[str, List[ResolvedImport]]) -> CodeGraph:
    graph = CodeGraph()
    ...
    for pf in parsed_files:
        ...
        for r_imp in resolved_map.get(pf.module_name, []):
            if r_imp.status == "external" and r_imp.target_module is not None:
                ...
                if (r_imp.target_module != pf.module_name
                        and r_imp.target_module in parsed_module_names):
                    internal_edges.append(ImportEdge(
