...
def main():
    ...
    parser.add_argument("--format", choices=["text", "json", "mermaid", "tree"], default="text", help="Output format")
    ...
    parser.add_argument("--query", help="Semantic search query to co-rank files by relevance")
    parser.add_argument("--neighborhood", help="Show the local neighborhood of a module name or symbol")
    parser.add_argument("--depth", type=int, default=1, help="Depth of neighborhood traversal (default: 1)")
    parser.add_argument("--max-chars", type=int, help="Limit output size by cumulative character count")
    parser.add_argument("--python-only", action="store_true", help="Skip scanning C++ source files")
    parser.add_argument("--include-docstrings", action="store_true", help="Include first-line docstrings in text output")
    parser.add_argument("--show-rank", action="store_true", help="Show PageRank scores in tree output")
    ...
    if args.neighborhood:
        ...
        if args.neighborhood in graph.nodes:
            ...
            if found_module:
                ...
                print(f"Could not find module or symbol: {args.neighborhood}", file=sys.stderr)
                return
    ...
    if args.max_chars is not None and len(output) > args.max_chars:
        ...
        sorted_modules = sorted(graph.nodes.keys(), key=lambda m: pagerank.get(m, 0), reverse=True)
        ...
        while len(sorted_modules) > 1:
            ...
            if args.format == "text":
                ...
                output = sub_exporter.export(top_n=args.top_n, include_docstrings=args.include_docstrings)
            elif args.format == "json":
