...
class TextMapExporter:
    ...
    def export(self, top_n: int = None, include_docstrings: bool = False) -> str:
        sorted_modules = sorted(
            self.graph.nodes.keys(), key=lambda m: self.pagerank.get(m, 0), reverse=True
        )
        ...
        def _render_tree(current_dict, prefix=""):
            ...
            for i, (item_type, key, val) in enumerate(items):
                ...
                if item_type == "folder":
                    ...
                    if show_rank:
                        rank_str = (
                            f" [rank: {self.pagerank.get(node.module_name, 0):.4f}]"
                        )
                    ...
                    if not is_low_relevance:
                        sym_lines = _render_symbols(
                            node, next_prefix, include_docstrings
                        )
