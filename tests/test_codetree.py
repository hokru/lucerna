...
def test_search_and_combine():
    from codetree.graph import CodeGraph, FileNode
    from codetree.parser import ParsedSymbol
    from codetree.search import score_query, combine_scores
    
    graph = CodeGraph()
    node1 = FileNode("pkg/utils.py", "pkg.utils", [ParsedSymbol("helper", "function", 1, "()", "Helps with something", [], None)], "Utilities", [], [])
    node2 = FileNode("pkg/core.py", "pkg.core", [ParsedSymbol("MyClass", "class", 1, "()", "Core class", [], None)], "Core logic", [], [])
...
def test_tree_exporter():
    from codetree.graph import CodeGraph, FileNode
    from codetree.parser import ParsedSymbol
    from codetree.exporter import TreeExporter
    
    graph = CodeGraph()
    node1 = FileNode("pkg/utils.py", "pkg.utils", [ParsedSymbol("helper", "function", 1, "()", "Helps", [], None)], None, [], [])
    graph.add_node(node1)
...
def test_json_exporter_fields():
    from codetree.graph import CodeGraph, FileNode
    from codetree.exporter import JsonExporter
...
def test_parser_constants_and_all():
    import ast
    from codetree.parser import CodeVisitor, ParsedSymbol
